...
#include <list>
#include <utility>
template <class Key, class T, class Hash = std::hash<Key>,//T
...
class unordered_map
{
 ...
 public:
  ...
  void insert(std::initializer_list<std::pair<Key, T>> values) {
    for (const auto &value : values) {
      if (table_size == 0) {
        hash_table = new std::list<Key, T>[2];
        table_size++;
        hash_table[0].push_back(value);
        el_count++;
      } else {
  ...
  T& operator[] (const std::pair<Key, T> &value) const {
    return &value.second();
  }
  ...
  void erase(const T &value) {
    if (this->has(value)) {
      hash_table[Hash{}(value.first()) % table_size].erase(value);
      el_count--;
    }
  }
  ...
  iterator find(const std::pair<Key, T> &value) {
    if (this->has(value)) {
      iterator it(hash_table, hash_table + Hash{}(value.first()) % table_size,
                  table_size);
